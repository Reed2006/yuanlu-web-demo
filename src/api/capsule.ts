...
export const capsuleApi = {
  /** 获取附近胶囊 */
  getNearby: async (lat: number, lng: number, radius = 500) => {
    const response = await get<{ items: CapsuleData[] }>(`/capsule/nearby?lat=${lat}&lng=${lng}&radius=${radius}`);
    return response;
  },
  ...
  getMine: async (userId: string | number, scope: 'all' | 'created' | 'found' = 'all') => {
    return await get<{ items: CapsuleMineItem[] }>(`/capsule/mine?user_id=${userId}&scope=${scope}`);
  },
  ...
  getDetail: async (capsuleId: string | number, viewerUserId?: string | number) => {
    ...
    return await get<CapsuleDetailData>(`/capsule/${capsuleId}${query}`);
  },
